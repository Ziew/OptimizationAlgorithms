...
public class CalculateProbabilityFeromon {
	...
	public CalculateProbabilityFeromon(Matrix matrix, CollectionPointInformation cPI, double betha,int numberOfElement, int numberOfValues, double p){
	...
	public void calculateProbability(){
		...
		for (int i = 0; i < numberOfElements_; i++){
			...
			for(int z = 0 ; z < numberOfValues_; z++){
				denominator = denominator + (Math.pow(cPI_.getPointsInformation(i, z).getFeromon(),betha_));
